...
public class UpdateServer extends Activity {
	...
	private static final String rssFeed = "http://www.txtease.com/android/push/login/update.php";
	...
	public void init() {
		...
		spinner.setSelection(adapter.getPosition(location));
	...
	class MyTask extends AsyncTask<String, Void, JSONObject> {
		...
		protected JSONObject doInBackground(String... params) {
			...
			String paramx = "id=" + eventID + "&date" + update_date
					...
					+ "&deleteEvent=" + delete_event +"&addbck=" + addBack;
...
String note ="";
			...
			try {
				...
				if (result.length()>0){
			JSONObject jsonx = json;

			System.out.println(json.toString());
			 note = json.getString("msg");
			...
			if(note.length()>0){
				...
				Toast.makeText(UpdateServer.this, note, Toast.LENGTH_LONG)
				.show();
